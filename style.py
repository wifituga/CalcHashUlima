...
def hashing():
    if (formato.get() == "Texto"):
        ...
        label_MD5 = tk.Label(width = 57, bg="White", text = md5_hash ,borderwidth=2)
        ...
        label_SHA256 = tk.Label(width = 60, bg="White", text = sha256.hexdigest() ,borderwidth=2)
        ...
        with open(file, 'rb') as archivo :
            ...
            label_MD5 = tk.Label(width = 57, bg="White", text = md5.hexdigest() ,borderwidth=2)
            label_MD5.place(x=120, y=80)
...
recgen = tk.Frame(window, width=535, height=485,
...
input_txt= tk.Entry(width=70, borderwidth=1.5)
...
# Parte MD5
MD5_check = tk.Checkbutton(text="MD5", bg = "Light grey", variable = var1, onvalue=1, offvalue=0)
...
# Parte SHA256
SHA256_check = tk.Checkbutton(text="SHA256", bg = "Light grey", variable = var2, onvalue=1, offvalue=0)
